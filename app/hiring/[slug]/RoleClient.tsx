...
import { ArrowLeft, Clock, ExternalLink, Lock, MapPin } from "lucide-react";
      ...
      <div style={styles.body}>
        ...
        <div style={styles.roleCard}>
          ...
          <p style={styles.datesText}>
            ...
            {" · "}Deadline: {closesAt.toLocaleDateString("en-US", { day: "numeric", month: "short", year: "numeric" })}
          </p>
          ...
          <div style={styles.tagRow}>
            ...
            {role.job_type && (
              <span style={styles.tag}>
                {role.job_type.split("_").map((w: string) => w.charAt(0).toUpperCase() + w.slice(1)).join(" ")}
              </span>
            )}
            {role.job_type === "contract" && role.contract_duration_value && (
              <span style={styles.tag}>{role.contract_duration_value} {role.contract_duration_unit}</span>
            )}
            {role.compensation_amount && (
              <span style={styles.tag}>
                {getCurrencySymbol(role.compensation_currency)}{role.compensation_amount}
                {role.pay_frequency && role.pay_frequency !== "fixed" ? ` / ${role.pay_frequency.replace("ly", "")}` : ""}
              </span>
            )}
        ...
        <ApplicationForm
          roleId={role.id}
          questions={questions}
          cutoffScore={role.cutoff_score}
          applicationFields={role.application_fields}
        />
      </div>
    </div>
  );
...
const styles: Styles = {
  ...
  roleTitle: { fontSize: "22px", fontWeight: "700", color: "#111111", margin: "0 0 8px 0" },
  datesText: { fontSize: "11px", color: "#bbbbbb", margin: "0 0 14px 0" },
